...
type RequiredFieldId = 'requester' | 'email' | 'content';
...
export class SupportWidgetElement extends HTMLElement {
        ...
        if (config.toastDuration !== undefined) {
            ...
            if (typeof duration === 'number') {
                ...
                if (duration.success !== undefined) {
                    this.toastSuccessDuration = this.normalizeToastDuration(
                        duration.success,
                    );
                } else {
                ...
                if (duration.error !== undefined) {
                    this.toastErrorDuration = this.normalizeToastDuration(
                        duration.error,
                    );
                } else {
    ...
    open(): void {
        ...
        this.dispatchEvent(
            new CustomEvent(WIDGET_EVENTS.OPEN, { bubbles: true }),
        );
    }
    ...
    close(): void {
        ...
        this.dispatchEvent(
            new CustomEvent(WIDGET_EVENTS.CLOSE, { bubbles: true }),
        );
    }
                    ...
                    if (parsed && typeof parsed === 'object') {
                        if (parsed.success !== undefined) {
                            this.toastSuccessDuration =
                                this.normalizeToastDuration(
                                    Number(parsed.success),
                                );
                        }
                        if (parsed.error !== undefined) {
                            this.toastErrorDuration =
                                this.normalizeToastDuration(
                                    Number(parsed.error),
                                );
                        }
    ...
    private bindActions(): void {
        ...
        root.getElementById('submit-btn')?.addEventListener(
            'click',
            () => void this.submit(),
        );
        root.getElementById('toast-close-btn')?.addEventListener(
            'click',
            () => {
                this.hideToast();
            },
    ...
    private setupPreviewControls(): void {
        ...
        root.getElementById('preview-close')?.addEventListener('click', () =>
            this.closePreview(),
        );
    ...
    private setupToastCopy(): void {
        ...
        const copyText = async (text: string, btn: HTMLElement | null) => {
            ...
            if (navigator.clipboard && window.isSecureContext) {
                try {
                    ...
                    console.warn(
                        'navigator.clipboard.writeText failed, trying fallback: ',
                        err,
                    );
                }
            }
        ...
        copyLinkBtn?.addEventListener('click', () => {
            const linkEl = root.getElementById(
                'toast-link',
    ...
    private setFieldError(id: RequiredFieldId | 'phone-number'): void {
    ...
    private showToast(payload: ToastPayload, autoCloseMs?: number): void {
        ...
        const requestCodeEl =
            this.shadowRoot?.getElementById('toast-request-code');
    ...
    private startToastCountdown(
        ...
        const updateCountdown = (): void => {
            const seconds = Math.max(
                1,
                Math.ceil(this.toastRemainingMs / 1000),
            );
            if (secondsEl) {
            ...
            const percent = Math.max(
                0,
                Math.min(1, this.toastRemainingMs / this.toastTotalMs),
            );
    ...
    private validateRequiredFields(): boolean {
        ...
        // Validate phone number format only if user entered a value
        const phone = this.getInputValue('phone-number');
        if (phone && !PHONE_REGEX.test(phone)) {
            this.setFieldError('phone-number');
            if (!firstInvalidId) {
                firstInvalidId = 'phone-number';
            }
        }

        if (firstInvalidId) {
            const firstInvalid = this.shadowRoot?.getElementById(
                firstInvalidId,
    ...
    private setSubmitting(isSubmitting: boolean): void {
        ...
        const submitBtn = root.getElementById(
            'submit-btn',
        ) as HTMLButtonElement | null;
        const cancelBtn = root.getElementById(
            'cancel-btn',
        ) as HTMLButtonElement | null;
        const closeBtn = root.getElementById(
            'close-btn',
        ) as HTMLButtonElement | null;
